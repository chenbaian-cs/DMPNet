...
from networks.DMPN import build_model
import numpy as np
import os
import cv2
import time
...
class Solver(object):
    def __init__(self, train_loader, test_loader, config):
        ...
        if config.mode == 'train':
            ...
            if config.resume == True:
                if config.log_dir is not None and os.path.isfile(config.log_dir):
                    checkpoint = torch.load(config.log_dir)
                    self.net.load_state_dict(checkpoint['model_state_dict'])
                    self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                    self.start_epoch = checkpoint['epoch']
                    print("-> loaded checkpoint %s (epoch: %d)" % (config.log_dir, self.start_epoch))
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epoch):
            ...
            torch.save(save_dict, '%s/epoch_%d.tar' % (self.config.save_folder, epoch + 1))
